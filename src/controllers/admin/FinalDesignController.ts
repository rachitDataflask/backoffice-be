...
class FinalDesignController {
    ...
    try {
      ...
      if (calculation_type === "SUB_BUILDING") {
        finalDesign = await SubBuildingCalculation.findOne({
          ...
          sub_service_id,
          action_id,
        } as ISubBuildingCalculation).lean();
      }
  ...
  static editFinalDesign = async (
    ...
    try {
      if (calculation_type === "BUILDING") {
        finalDesign = await BuildingCalculation.findByIdAndUpdate(
          id,
          req.body,
          {
            new: true,
      ...
      } else if (calculation_type === "SUB_BUILDING") {
        finalDesign = await SubBuildingCalculation.findByIdAndUpdate(
          id,
          req.body,
          {
            new: true,
      ...
      } else if (calculation_type === "LEVEL") {
        finalDesign = await LevelCalculation.findByIdAndUpdate(id, req.body, {
          new: true,
        });
      } else if (calculation_type === "ROOM") {
        finalDesign = await RoomCalculation.findByIdAndUpdate(id, req.body, {
          new: true,
        });
      }
      ...
      if (!finalDesign) {
        throw new NotFoundError(
          "Calculation type is invalid or design not found."
        );
      }
