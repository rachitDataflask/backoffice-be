...
class SubBuildingController {
  static listAll = async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      if (building_id) {
        subBuildings = await SubBuilding.find({ building_id }).populate(
          "building_id",
          "type"
        );
      } else {
        subBuildings = await SubBuilding.find().populate("building_id", "type");
      }
  ...
  static getOneById = async (
    ...
    try {
      const subBuilding = await SubBuilding.findById(id).populate(
        "building_id",
        "type"
      );
      if (!subBuilding)
  ...
  static newSubBuilding = async (
    ...
    try {
      const building = SubBuilding.build({
        building_id,
        type,
        description,
      } as ISubBuilding);
      ...
      console.error(e);
      const error = e as Error.ValidationError;
      next(new ClientError(processErrors(error)));
    }
  };
